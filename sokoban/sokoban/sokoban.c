...
int move_char(char* map, int pos_x, int pos_y, int move, int* step) {
    if (map[PLAYER_POSITION + move] != '#') {                   // if the player doesn't hit the wall
        ...
        if (map[PLAYER_POSITION + move] == 'B' || map[PLAYER_POSITION + move] == '0') {         // if the player pushes box
            ...
            if (map[PLAYER_POSITION + move] == ' ') {           // if the box goes to empty space
                ...
                if (map[PLAYER_POSITION + move + move] == '#') {
                    // need to fix here again!
                    return ESC;
                }
            }
...
int game_play(char* map) {
    ...
    while (key != ESC) {                                                    // game loop, 27 is a integer representation of esc
        ...
        switch (key) {
            case 'w':
                key = move_char(map, pos_x, pos_y, UP, &steps);                   // move character up
                steps++;
                break;
            case 's':
                key = move_char(map, pos_x, pos_y, DOWN, &steps);                 // move character down
                steps++;
                break;
            case 'a':
                key = move_char(map, pos_x, pos_y, LEFT, &steps);                 // move character to the left
                steps++;
                break;
            case 'd':
                key = move_char(map, pos_x, pos_y, RIGHT, &steps);                // move character to the right
                steps++;
                break;
            default:
                break;
